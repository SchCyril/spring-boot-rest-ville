...
import fr.diginamic.hello.controller.VilleController;
...
public class VilleControllerImpl implements VilleController {

    private final IVilleService villeService;

    public VilleControllerImpl(VilleServiceImpl villeServiceImpl) {
        this.villeService = villeServiceImpl;
    }
    ...
    @GetMapping("/{popMin}")
    @Override
    public void exportCSV(@PathVariable Integer popMin, HttpServletResponse response) throws ExceptionFonctionnelle, IOException,
            DocumentException {
        villeService.exportCsvVillesSupMin(popMin, response);
    }
}
